...
type Props = {
  ...
  onToggle?: (serviceId: string) => void; // alterna selección
  maxSelectable?: number;                 // default: 3

  /** Alto opcional del contenedor scroll */
...
export default function ServiceList({
  ...
  const groups = useMemo(() => {
    ...
    return Object.entries(byCat)
      ...
      .map(([cat, arr]) => [cat, arr.sort((a, b) => a.name.localeCompare(b.name))] as const);
  }, [services]);
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white rounded-xl shadow border overflow-hidden">
      <div className={cn("relative overflow-y-auto", heightClassName)}>
        {groups.map(([categoryName, items]) => (
          <section key={categoryName}>
            <div className="sticky top-0 z-10 bg-gray-50/95 backdrop-blur border-b border-gray-200 px-4 py-2">
              <h2 className="font-semibold text-gray-800 tracking-wide">{categoryName}</h2>
            </div>
