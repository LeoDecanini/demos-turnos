...
export default async function BookingPublicPage({
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-amber-50/30 grid place-items-center px-6 py-24">
        <Card className="max-w-xl w-full border-0 shadow-xl bg-white">
          ...
          <CardContent className="text-gray-600">
            No pudimos cargar los datos de {esGrupo ? "las reservas" : "la reserva"}. Verificá el enlace o intentá nuevamente
            más tarde.
          </CardContent>
        </Card>
      </div>
  ...
  const groupCurrency =
    (esGrupo && groupData?.totals ? groupData.totals.currency : booking.depositCurrency) || booking.currency;
  ...
  const deadlineHuman = depositDeadline
    ? new Intl.DateTimeFormat("es-AR", { dateStyle: "medium", timeStyle: "short" }).format(depositDeadline)
    : null;
  ...
  const isCanceled = (booking.status || "").toLowerCase() === "canceled" || booking.depositStatus === "expired";

  const gcalUrl = buildGoogleCalendarUrl({
    title: `${service?.name || "Reserva"}${
      professional?.name && professional.name !== "Indistinto" ? ` — ${professional.name}` : ""
    }${esGrupo ? ` (Grupo de ${groupData?.bookings?.length || 1} reservas)` : ""}`,
