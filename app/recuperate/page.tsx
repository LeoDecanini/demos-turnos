...
export default function RecoverPage() {
  ...
  const requestCode = async () => {
    ...
    try {
      setPending(true);
      setErr(null);
      const r = await fetch(`${API}/bookingmodule/public/clients/forgot-password/start`, {
        method: 'POST',
  ...
  const confirmReset = async () => {
    ...
    try {
      setPending(true);
      setErr(null);
      const r = await fetch(`${API}/bookingmodule/public/clients/forgot-password/confirm`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
          ...
          <div className="rounded-2xl bg-white shadow-xl ring-1 ring-slate-100 p-6 md:p-8">
            ...
            <div className="space-y-5">
              ...
              {!requested ? (
                <button
                  onClick={requestCode}
                  disabled={!/\S+@\S+\.\S+/.test(email) || pending}
                ...
                <>
                  ...
                  <button
                    ...
                    disabled={!/^\d{8}$/.test(code) || password.length < 6 || pending}
                    className="w-full rounded-xl bg-yellow-400 text-slate-900 font-semibold py-3 shadow hover:bg-yellow-500 disabled:opacity-60 disabled:cursor-not-allowed transition"
                  >
