...
export default function RecoverPage() {
    ...
    const requestCode = async () => {
        ...
        try {
            ...
            // NUEVO endpoint correcto con :email en la URL (sin body obligatorio)
            const r = await fetch(
                `${API}/bookingmodule/public/clients/${encodeURIComponent(email)}/start-password-reset`,
                { method: 'POST', headers: { 'Content-Type': 'application/json' } }
            );

            if (!r.ok) throw new Error((await r.text()) || 'No se pudo enviar el código');

            // En dev puede venir { ok: true, devCode: '12345678' }
    ...
    const confirmReset = async () => {
        ...
        try {
            ...
            // NUEVO endpoint correcto con :email + body { code, newPassword }
            const r = await fetch(
                `${API}/bookingmodule/public/clients/${encodeURIComponent(email)}/reset-password`,
                {
                    method: 'POST',
